...
function checkZodiacFromDate($dBirthdate)
{
    ...
    $date = DateTime::createFromFormat('Y-m-d', $dBirthdate);

    $date_errors = DateTime::getLastErrors();
    if ($date_errors['warning_count'] + $date_errors['error_count'] > 0) {
        //$errors[] = 'Some error occurred. Please try again.';
    ...
    $xml = simplexml_load_file(__DIR__ . "/../utils/dates/dates.xml") or die("Error: Cannot create object");

    //build zodiac date ranges
    /**
     * sFirstMonth, sFirstDay => the begining of a specific zodiac range
     * sLastMonth, sLastDay => the end of a specific zodiac range
     */
    for ($i = 1; $i <= 12; $i++) {
